                ...
                <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 h-fit transition-colors">
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                        <div className="space-y-2">
                            <label className="text-[10px] font-black uppercase tracking-wider text-slate-400 dark:text-slate-500">Zip Code</label>
                            <input
                                {...register("zipCode", { required: true, minLength: 5, maxLength: 5 })}
                                className="w-full p-3 bg-slate-50 dark:bg-slate-950 border dark:border-slate-800 rounded-lg font-bold text-[#024653] dark:text-white outline-none focus:border-[#05D16E] transition-colors"
                                placeholder="90210"
                        ...
                        <div className="space-y-2">
                            <label className="text-[10px] font-black uppercase tracking-wider text-slate-400 dark:text-slate-500">City Name</label>
                            <input
                                {...register("city")}
                                className="w-full p-3 bg-slate-50 dark:bg-slate-950 border dark:border-slate-800 rounded-lg font-bold text-[#024653] dark:text-white outline-none focus:border-[#05D16E] transition-colors"
                        ...
                        <div className="flex gap-2 pt-4">
                            {isEditing && (
                                <button type="button" onClick={cancelEdit} className="flex-1 p-3 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-300 font-bold rounded-xl hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
                                    Cancel
                                </button>
                            )}
                            <button
                                ...
                                className="flex-1 p-3 bg-[#024653] dark:bg-[#0E6168] text-white font-bold rounded-xl hover:bg-[#02333d] dark:hover:bg-[#0a484d] transition-colors flex justify-center items-center gap-2"
                            >
                ...
                <div className="lg:col-span-2 bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 transition-colors">
                    ...
                    {isLoading ? (
                        <div className="space-y-4 animate-pulse">
                            {[1, 2, 3].map(i => <div key={i} className="h-16 bg-slate-50 dark:bg-slate-800 rounded-xl" />)}
                        </div>
                    ) : areas.length === 0 ? (
                                ...
                                <div key={area.id} className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800 hover:border-slate-200 dark:hover:border-slate-700 transition-all group">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">{area.city || "Unknown City"}</p>
                                        </div>
                                    </div>

                                    <div className="flex items-center gap-2 opacity-100 lg:opacity-0 group-hover:opacity-100 transition-opacity">
                                        <span className={`px-2 py-1 rounded-md text-[10px] font-black uppercase tracking-widest ${area.status === 'active' ? 'bg-[#05D16E]/10 text-[#05D16E]' : 'bg-amber-100 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400'}`}>
