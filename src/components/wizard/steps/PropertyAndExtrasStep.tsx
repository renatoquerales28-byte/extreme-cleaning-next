            ...
            <div className={`max-w-6xl mx-auto w-full px-6 grid grid-cols-1 ${!(data.cleaningType === "move_in_out" || data.cleaningType === "post_construction") ? 'lg:grid-cols-2' : ''} gap-10 items-stretch`}>
                ...
                <div className="flex flex-col space-y-4">
                    <div className="flex items-center gap-3 ml-1">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-[0.25em] text-[#024653]">1. Property Intelligence</span>
                    </div>
                    ...
                    <div className="flex-1 bg-[#F9F8F2] border border-[#024653]/10 rounded-2xl p-6 lg:p-8 space-y-6 flex flex-col justify-center">
                        <div className="bg-white border-2 border-[#024653]/5 rounded-xl p-6 shadow-sm group hover:border-[#024653]/20 transition-all">
                            <div className="flex items-center gap-4 mb-3">
                                ...
                                <span className="text-[10px] font-semibold uppercase tracking-widest text-[#024653]/30">Total Liveable Area</span>
                            </div>
                            <div className="flex items-baseline gap-3">
                                <input
                                    ...
                                    type="number"
                                    placeholder="2400"
                                    className="bg-transparent text-4xl font-bold text-[#024653] outline-none w-full tabular-nums"
                                />
                                <span className="text-[12px] font-bold text-[#024653]/20 uppercase tracking-[0.3em] whitespace-nowrap">Sq. Ft.</span>
                            </div>
                        </div>
                            ...
                            {[
                                ...
                                <div key={spec.id} className="bg-white border-2 border-[#024653]/5 rounded-xl p-4 flex flex-col items-center gap-3 shadow-sm group hover:border-[#024653]/20 transition-all">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-[9px] font-bold uppercase tracking-widest text-[#024653]/30 group-hover:text-[#024653] transition-colors">{spec.label}</span>
                                    </div>
                                    <div className="flex items-center justify-between w-full bg-[#024653]/5 rounded-xl p-2 px-4">
                                        <button
                                            type="button"
                                            onClick={() => adjust(spec.id as any, -1)}
                                            className="flex items-center justify-center text-[#024653] hover:text-red-500 transition-all active:scale-90"
                                        >
                                            <Minus size={18} />
                                        </button>
                                        <span className="text-2xl font-bold text-[#024653] tabular-nums">
                                            {data[spec.id as "bedrooms" | "bathrooms"] || 1}
                                        </span>
                                        <button
                                            type="button"
                                            onClick={() => adjust(spec.id as any, 1)}
                                            className="flex items-center justify-center text-[#024653] hover:text-[#05D16E] transition-all active:scale-90"
                                        >
                                            <Plus size={18} />
                                        </button>
                                    </div>
                ...
                {!(data.cleaningType === "move_in_out" || data.cleaningType === "post_construction") && (
                    <div className="flex flex-col space-y-4">
                        <div className="flex items-center gap-3 ml-1">
                            ...
                            <span className="text-[10px] font-bold uppercase tracking-[0.25em] text-[#024653]">2. Premium Extras</span>
                            {extras.length > 0 && (
                                <div className="ml-auto flex items-center gap-2 px-3 py-1 bg-[#024653] rounded-full">
                                    <ShieldPlus size={10} className="text-[#05D16E]" />
                                    <span className="text-[8px] font-bold uppercase text-white tracking-widest">{extras.length} Selected</span>
                                </div>
                            )}
                        </div>
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className="text-center">
                                                <p className="text-[10px] font-bold uppercase tracking-tight leading-none mb-1">{extra.label}</p>
                                                <p className={`text-[7px] font-semibold uppercase tracking-widest opacity-40 leading-none ${isSelected ? "text-[#05D16E]" : ""}`}>
                                                    Elite Add-on
                                                </p>
                                            </div>
                                            {isSelected && (
            ...
            {!(data.cleaningType === "move_in_out" || data.cleaningType === "post_construction" || data.cleaningType === "first_cleaning") && (
                <div className="max-w-6xl mx-auto w-full px-6 mt-10">
                    <div className="flex items-center gap-3 ml-1 mb-4">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-[0.25em] text-[#024653]">3. Service Schedule</span>
                    </div>
                            ...
                            {[
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <span className={`text-[8px] font-semibold uppercase tracking-tighter ${isSelected ? "text-[#05D16E]" : "text-[#024653]/30"}`}>
                                            {freq.offer}
                                        </span>
                                    </button>
                                );
