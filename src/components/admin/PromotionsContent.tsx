...
import { createPromotion, togglePromotion, deletePromotion, getPromotions } from "@/app/actions/admin";
...
export default function PromotionsContent({ initialPromotions }: PromotionsContentProps) {
    ...
    // We can re-fetch to ensure sync after complex operations if needed,
    // but for now we'll stick to mostly optimistic updates or simple state management
    // since the server component provided initial state.

    // Helper to refresh list from server action if needed manually
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        if (result.success) {
            setNewCode("");
            // Refresh list from server to get the new ID and correct order
