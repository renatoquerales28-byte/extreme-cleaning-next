...
export default function ExtremeCleaningWizard() {
    ...
    const methods = useForm<WizardData>({
        ...
        defaultValues: {
            step: 0, // Legacy field, kept for schema compatibility
            zipCode: urlZip || "",
    ...
    useEffect(() => {
        if (mode === "returning") {
            ...
            // Check if we can jump deeper? Ideally engine handles this via history
            // For now, let's trust the engine's default flow or allow specific jumps locally
            // But usually starting at Zip is safest if no data is confirmed.
        }
    }, [mode, urlZip, goToStep]);

    // Database Warm-up (Preserved from original)
    useEffect(() => {
        const warmUp = async () => {
    ...
    // Handlers for specific step interactions
    const handleReturningFound = (customerData: any) => {
        // Hydrate form if needed?
        goNext();
    };

    const handlePropertySelect = (id: number) => {
        if (id === 0) {
            // New Property -> Residential Flow
            methods.setValue("serviceType", "residential");
            goToStep("service"); // Or cleaning_type depending on preference
        } else {
            goNext();
        }
    };
                ...
                <div className={`w-full h-screen fixed inset-0 flex flex-col lg:flex-row bg-[#F9F8F2] overflow-hidden ${inter.className}`}>
                    ...
                    <div className="hidden lg:flex w-1/3 bg-[#024653] relative flex-col justify-between p-12 text-white overflow-hidden shrink-0 border-r-4 border-[#10f081]">
                        <AnimatePresence mode="wait">
                            <motion.div
                                key={stepId} // Key triggers animation on step change
                        ...
                        <AnimatePresence mode="wait" initial={false}>
                            ...
                            >
                                <CurrentStepComponent
                                    onNext={goNext}
                                    onBack={goBack} // Pass just in case
                                    onReturning={() => goToStep('returning_lookup')}
                                    onFound={handleReturningFound} // For returning lookup
                                    onSelect={handlePropertySelect} // For returning select
                                    onEditStep={(id: StepId) => goToStep(id)} // For review step
                                    totalPrice={totalPrice} // For price step
                                    onSubmit={goNext} // For final step
                                />
                            </motion.div>
                        </AnimatePresence>
