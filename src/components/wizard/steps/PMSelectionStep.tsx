            ...
            <div className="max-w-6xl mx-auto w-full px-6 space-y-10">
                ...
                <div className="flex flex-col space-y-4">
                    <div className="flex items-center gap-3 ml-1">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-[0.25em] text-[#024653]">1. Current Property Location</span>
                        <div className="ml-auto flex items-center gap-2 px-3 py-1 bg-[#024653]/5 rounded-full">
                            <span className="text-[8px] font-bold uppercase text-[#024653]/40 tracking-widest">ZIP: {data.zipCode}</span>
                        </div>
                    </div>
                    ...
                    <div className="bg-[#F9F8F2] border border-[#024653]/10 rounded-2xl p-6 lg:p-10 grid grid-cols-1 lg:grid-cols-2 gap-6 items-center">
                        <div className="space-y-4">
                            <div className="space-y-2">
                                <label className="text-[9px] font-semibold uppercase tracking-widest text-[#024653]/40 ml-1">Service Address</label>
                                <input
                                    {...register("address")}
                                    placeholder="123 Example St"
                                    className="w-full bg-white border border-[#024653]/10 rounded-xl p-4 text-sm font-semibold text-[#024653] outline-none focus:border-[#05D16E] transition-all"
                                />
                            </div>
                            <div className="space-y-2">
                                <label className="text-[9px] font-semibold uppercase tracking-widest text-[#024653]/40 ml-1">City</label>
                                <input
                                    {...register("city")}
                                    placeholder="Spokane"
                                    className="w-full bg-white border border-[#024653]/10 rounded-xl p-4 text-sm font-semibold text-[#024653] outline-none focus:border-[#05D16E] transition-all"
                        ...
                        <div className="bg-[#024653] text-white rounded-2xl p-8 flex flex-col items-center justify-center text-center space-y-4 shadow-xl shadow-[#024653]/20">
                            ...
                            <div className="space-y-1">
                                <h3 className="text-sm font-bold uppercase tracking-widest">Adding another Property?</h3>
                                <p className="text-[10px] opacity-40 uppercase tracking-widest font-semibold">Save this location and start the next specs.</p>
                            </div>
                            <button
                                type="button"
                                onClick={handleAddAnother}
                                className="bg-[#05D16E] hover:bg-[#05D16E]/90 text-[#024653] px-8 py-3 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all shadow-lg active:scale-95 flex items-center gap-3"
                            >
                ...
                {portfolio.length > 0 && (
                    <div className="flex flex-col space-y-4">
                        <div className="flex items-center gap-3 ml-1">
                            ...
                            <span className="text-[10px] font-bold uppercase tracking-[0.25em] text-[#024653]">2. Portfolio Queue ({portfolio.length})</span>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {portfolio.map((prop) => (
                                <div key={prop.id} className="bg-white border-2 border-[#024653]/5 rounded-2xl p-6 flex flex-col gap-4 relative group hover:border-[#05D16E]/20 transition-all">
                                    ...
                                    <div className="space-y-1">
                                        <p className="text-[8px] font-bold uppercase tracking-widest text-[#05D16E]">{prop.cleaningType}</p>
                                        <h4 className="text-xs font-bold text-[#024653] uppercase tracking-tight truncate pr-6">{prop.address}</h4>
